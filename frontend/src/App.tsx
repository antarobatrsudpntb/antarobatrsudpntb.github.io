...
function KurirView({ active, data, user, incident, operationalOptions, onRefresh, onMutation, navigate, show }: { active: string; data: Row[]; user: AppUser; incident: Row | null; operationalOptions: OperationalOptions; onRefresh: () => void; onMutation: (result: Row) => void; navigate: (id: string) => void; show: (type: "success" | "error", message: string) => void }) {
  ...
  const pendingModal = pendingRow ? <Modal title="Pending Pengantaran" text={deliveryCode(pendingRow) + " • " + deliveryName(pendingRow)} onClose={() => !busy && setPendingRow(null)}>
    <form className="modal-form" onSubmit={async (event) => {
      ...
      <footer className="modal-actions"><button type="button" className="secondary-button" disabled={busy} onClick={() => setPendingRow(null)}>Batal</button><button className="warning-button" disabled={busy || !pendingReason}><Clock3 /> Simpan Pending</button></footer>
    </form>
  </Modal> : null;
...
function AdminView({ active, data, areas, areaCount, transactionCount, activeAccountCount, accounts, retention, health, currentUser, operationalSettings, onRefresh, onLogout, navigate, show }: { active: string; data: Row[]; areas: Row[]; areaCount: number; transactionCount: number; activeAccountCount: number; accounts: Row[]; retention: Row | null; health: Row; currentUser: AppUser; operationalSettings: Row; onRefresh: () => void; onLogout: () => void; navigate: (id: string) => void; show: (type: "success" | "error", message: string) => void }) {
  ...
  const healthOk = !healthHeld && ["AMAN", "NORMAL", "OK"].includes(healthStatus);
  const publicHealthStatus = healthHeld ? "Layanan Dihentikan Sementara" : healthOk ? "Layanan Berjalan Normal" : "Ada Pengaturan yang Perlu Ditinjau";
  const publicHealthText = healthHeld ? "Transaksi baru sedang ditahan sementara. Buka pengaturan Admin untuk meninjau kondisi layanan." : healthOk ? "MELESAT siap digunakan untuk operasional Farmasi dan Kurir." : "Buka pengaturan Admin untuk melihat bagian yang memerlukan peninjauan.";
