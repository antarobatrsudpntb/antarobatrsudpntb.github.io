...
import { installFastV2UiFeedback } from "./lib/fast-v2-ui";

installFastV2UiFeedback();
...
function statusOf(row: Row) {
  ...
  if (state === "PENDING") return "PENGANTARAN DITUNDA";
...
function stateVersionOf(row: Row) { return Math.max(0, Number(get(row, "stateVersion", "version", "resultVersion") || 0)); }
...
function uiStatusLabel(raw: unknown) {
  const status = String(raw || "").trim();
  const upper = status.toUpperCase();
  const labels: Record<string, string> = {
    PENDING: "PENGANTARAN DITUNDA",
    "MENUNGGU VERIFIKASI": "INFORMASI TERKIRIM",
    ACTIVE: "AKTIF",
    ELIGIBLE: "SIAP DITINJAU",
    BLOCKED: "BELUM SIAP",
    PREPARED: "SIAP DISETUJUI",
    APPROVED: "DISETUJUI",
    RUNNING: "SEDANG DIPROSES",
    COMPLETE: "SELESAI",
    CLEANED: "SELESAI",
  };
  return labels[upper] || status || "Belum ada status";
}

function statusSemanticKey(status: string, label: string) {
  const upper = `${status} ${label}`.toUpperCase();
  if (upper.includes("MENUNGGU OBAT KEMBALI")) return "menunggu-obat-kembali";
  if (upper.includes("PERLU TINDAK LANJUT")) return "perlu-tindak-lanjut";
  if (upper.includes("PENGANTARAN ULANG")) return "pengantaran-ulang-direncanakan";
  if (upper.includes("MENUNGGU AMBIL MANDIRI")) return "menunggu-ambil-mandiri";
  if (upper.includes("PENGANTARAN DITUNDA") || upper.includes("PENDING")) return "pengantaran-ditunda";
  if (upper.includes("KENDALA AKTIF")) return "kendala-aktif";
  if (upper.includes("MENUNGGU VERIFIKASI")) return "menunggu-verifikasi";
  if (upper.includes("TERVERIFIKASI")) return "terverifikasi";
  if (upper.includes("GAGAL ANTAR")) return "gagal-antar";
  if (upper.includes("TERKIRIM")) return "terkirim";
  if (upper.includes("DALAM PERJALANAN")) return "dalam-perjalanan";
  if (upper.includes("SIAP DIANTAR")) return "siap-diantar";
  if (upper.includes("MENUNGGU DIPROSES")) return "menunggu-diproses";
  return String(status || label).toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
}
function Badge({ status }: { status: string }) {
  const label = uiStatusLabel(status);
  return <span className={`status status--${statusSemanticKey(status, label)}`}>{label}</span>;
}
...
const navByRole: Record<Role, Array<{ id: string; label: string; icon: ReactNode }>> = {
  ...
  ADMIN: [
    ...
    { id: "settings", label: "Operasional & WA", icon: <MessageCircle /> }, { id: "archive", label: "Arsip & Backup", icon: <Archive /> },
  ],
  MANAJEMEN: [
...
function FarmasiView({ active, data, areas, incidents, operationalOptions, ensureAreas, onRefresh, onMutation, navigate, show }: { active: string; data: Row[]; areas: Row[]; incidents: Row[]; operationalOptions: OperationalOptions; ensureAreas: () => Promise<void>; onRefresh: () => void; onMutation: (result: Row) => void; navigate: (id: string) => void; show: (type: "success" | "error", message: string) => void }) {
  const [busy, setBusy] = useState(false);
  ...
  async function openEdit(row: Row) {
    try { await ensureAreas(); } catch (error) { show("error", error instanceof Error ? error.message : "Master wilayah gagal dimuat."); return; }
    ...
    setEditForm({
      ...
      areaKey: String(get(row, "serviceAreaId", "areaKey")), landmark: "",
      recipient: "", courierNote: [String(get(row, "landmark", "Patokan Lokasi")), String(get(row, "courierNote", "Catatan Kurir"))].filter(Boolean).filter((value, index, values) => values.indexOf(value) === index).join(" • "),
  ...
  async function scheduleRedelivery(row: Row) {
    ...
    try {
      ...
      onMutation(result);
      show("success", followupDate <= todayKey() ? "Pengantaran ke-2 telah diaktifkan dan masuk antrean Kurir." : `Pengantaran ulang dijadwalkan untuk ${followupDate}.`);
      void callFunction<Row>("prepareWhatsApp", { eventType: "REDELIVERY", id: rowId((result.record || row) as Row), attemptNo: get(result.attempt || {}, "attemptNo") || 2, scheduleDate: followupDate, snapshot: (result.record || row) as Row }).then(wa => { const d = whatsAppDialogFromResult("Pengantaran ulang siap", wa); if (d) setWaDialog(d); }).catch(() => undefined);
      setFollowupRow(null);
    } catch (error) {
  ...
  function todayActions(row: Row) {
    ...
    return <div className="action-row recovery-actions"><button className="secondary-button small" type="button" onClick={() => printRecord(row)}><Printer /> Cetak Label</button><button className="secondary-button small" type="button" disabled={busy} onClick={() => Number(get(row, "attemptCount", "Jumlah Percobaan") || 1) >= 2 ? act("prepareWhatsApp", { id: rowId(row), eventType: "REDELIVERY", attemptNo: 2 }, "Pesan pengantaran ulang disiapkan.", "Kirim ulang WA pengantaran ulang") : act("pharmacyRegistrationWaAction", { id: rowId(row) }, "Pesan kode penerimaan disiapkan.", "Kirim ulang kode penerimaan")}><MessageCircle /> {Number(get(row, "attemptCount", "Jumlah Percobaan") || 1) >= 2 ? "WA Antar Ke-2" : "WA Kode"}</button>{baseStatusOf(row) === STATUS.WAITING && <><button className="secondary-button small" type="button" disabled={busy} onClick={() => openEdit(row)}><Edit3 /> Edit</button><button className="primary-button small" type="button" disabled={busy} onClick={() => setReadyRow(row)}><PackageCheck /> Siap Diantar</button></>}{redeliveryDue && <button className="primary-button small" type="button" disabled={busy} onClick={() => openFollowup(row)}><CalendarDays /> Aktifkan Antar Ke-2</button>}</div>;
  }
  ...
  if (active === "register") return <div className="content-stack"><SectionTitle title="Daftar Pengantaran" text="Isi data minimum yang dibutuhkan Kurir. Informasi obat tidak dicantumkan." />
    <form className="form-card" onSubmit={register}><div className="form-grid">
      <Field label="Nomor Rekam Medis"><input ref={rmInputRef} required value={form.rm} onChange={(e) => setForm({ ...form, rm: e.target.value })} placeholder="Contoh: 00123456" /></Field>
      ...
      <Field label="Catatan alamat untuk Kurir (opsional)" hint="Isi hanya jika membantu Kurir menemukan alamat."><textarea value={form.courierNote} onChange={(e) => setForm({ ...form, courierNote: e.target.value })} placeholder="Contoh: gang samping masjid, pagar hijau" /></Field>
...
function KurirView({ active, data, user, incident, operationalOptions, onRefresh, onMutation, navigate, show }: { active: string; data: Row[]; user: AppUser; incident: Row | null; operationalOptions: OperationalOptions; onRefresh: () => void; onMutation: (result: Row) => void; navigate: (id: string) => void; show: (type: "success" | "error", message: string) => void }) {
  ...
  async function act(name: string, payload: Row, success: string, waTitle = "Pesan pasien siap") {
    ...
    try {
      ...
      onMutation(result); show("success", success);
      const waEvent = name === "setDeliveryPending" ? "PENDING" : name === "failDelivery" ? "FAILED" : "";
      if (waEvent) void callFunction<Row>("prepareWhatsApp", { eventType: waEvent, id: String(payload.id || rowId((result.record || {}) as Row)), attemptNo: get(result.attempt || result.record || {}, "attemptNo", "attemptCount") || 1, reason: waEvent === "PENDING" ? String(payload.reason || "") : "", snapshot: (result.record || {}) as Row }).then(wa => { const d = whatsAppDialogFromResult(waTitle, wa); if (d) setWaDialog(d); }).catch(() => undefined);
      return result;
    } catch (error) {
  ...
  function openWaEvent(eventType: "PENDING" | "FAILED", row: Row, title: string) {
    const attemptNo = Number(get(row.attempt || row, "attemptNo", "attemptCount") || 1);
    void callFunction<Row>("prepareWhatsApp", { eventType, id: rowId(row), attemptNo, snapshot: row }).then(wa => {
      const d = whatsAppDialogFromResult(title, wa); if (d) setWaDialog(d);
    }).catch(() => show("error", "WA belum dapat disiapkan. Pengantaran tetap dapat dilanjutkan."));
  }
  ...
  function taskActions(row: Row) {
    ...
    if (state === "PENDING") return <div className="action-row"><button className="secondary-button small" disabled={busy} onClick={() => openWaEvent("PENDING", row, "Pengantaran ditunda")}><MessageCircle /> Buka/Kirim Ulang WA</button><button className="primary-button small" disabled={busy} onClick={() => act("resumeDelivery", { requestId: requestId("resume"), id: rowId(row) }, "Pengantaran dilanjutkan.")}><Bike /> Lanjut Antar</button>{failButton}</div>;
    if (state === "RETURN_WAITING") return <div className="task-state-note"><TriangleAlert /><span>Gagal antar sudah dilaporkan. Kembalikan obat fisik ke Farmasi.</span><button className="secondary-button small" disabled={busy} onClick={() => openWaEvent("FAILED", row, "Gagal antar")}><MessageCircle /> Buka/Kirim Ulang WA</button></div>;
    return <div className="action-row"><button className="secondary-button small" disabled={busy} onClick={() => openPending(row)}><Clock3 /> Tunda</button>{failButton}<button className="primary-button small" disabled={busy} onClick={() => openComplete(row)}><Check /> Selesaikan</button></div>;
  }
  ...
  const claimModal = claimRow ? <ConfirmDialog
    ...
    onConfirm={async () => {
      const result = await act("claimTask", { requestId: requestId("claim"), id: rowId(claimRow) }, "Tugas berhasil diambil. Detail tujuan kini terbuka di Tugas Saya.");
  ...
  const pendingModal = pendingRow ? <Modal title="Tunda Pengantaran" text={deliveryCode(pendingRow) + " • " + deliveryName(pendingRow)} onClose={() => !busy && setPendingRow(null)}>
    <form className="modal-form" onSubmit={async (event) => {
      event.preventDefault();
      const result = await act("setDeliveryPending", { requestId: requestId("pending"), id: rowId(pendingRow), payload: { reason: pendingReason, detail: pendingDetail } }, "Pengantaran ditunda sementara; paket tetap dibawa Kurir.", "Pemberitahuan penundaan");
      ...
      <Field label="Alasan penundaan"><select required value={pendingReason} onChange={(event) => setPendingReason(event.target.value)}>{optionsOf(operationalOptions, "PENDING_REASONS").map(value => <option key={value}>{value}</option>)}</select></Field>
      ...
      <footer className="modal-actions"><button type="button" className="secondary-button" disabled={busy} onClick={() => setPendingRow(null)}>Batal</button><button className="warning-button" disabled={busy || !pendingReason}><Clock3 /> Simpan Penundaan</button></footer>
    </form>
  </Modal> : null;
  ...
  const failureModal = failureRow ? <Modal title="Catat Gagal Antar" text={deliveryCode(failureRow) + " • " + deliveryName(failureRow)} onClose={() => !busy && setFailureRow(null)}>
    <form className="modal-form" onSubmit={async (event) => {
      ...
      <footer className="modal-actions"><button type="button" className="secondary-button" disabled={busy} onClick={() => setFailureRow(null)}>Batal</button><button className="danger-button" disabled={busy || !failureReason}><TriangleAlert /> Simpan Gagal Antar</button></footer>
    </form>
  </Modal> : null;
  ...
  const resolveModal = resolveDialog && incident ? <Modal title="Selesaikan Kendala" text={String(get(incident, "type")) + " • laporan " + user.name} onClose={() => !busy && setResolveDialog(false)}>
    <form className="modal-form" onSubmit={async (event) => {
      ...
      <Field label="Catatan penyelesaian (opsional)"><textarea value={resolutionNote} onChange={(event) => setResolutionNote(event.target.value)} placeholder="Isi bila ada informasi tambahan" /></Field>
      <footer className="modal-actions"><button type="button" className="secondary-button" disabled={busy} onClick={() => setResolveDialog(false)}>Batal</button><button className="primary-button" disabled={busy}><Check /> Selesaikan Kendala</button></footer>
    </form>
  </Modal> : null;
  ...
  if (active === "incidents") return <div className="content-stack"><SectionTitle title="Kendala Perjalanan" text="Kendala adalah informasi operasional untuk koordinasi Kurir dan Farmasi; tidak memerlukan verifikasi." />{incident ? <article className="incident-focus"><span className="incident-visual"><TriangleAlert /></span><div><small>KENDALA AKTIF</small><h2>{get(incident, "type")}</h2>{get(incident, "detail") && <p>{get(incident, "detail")}</p>}<small>{get(incident, "delayEstimate") ? `Perkiraan keterlambatan: ${get(incident, "delayEstimate")}` : ""}</small><button className="primary-button" disabled={busy} onClick={() => { setResolutionNote(""); setResolveDialog(true); }}><Check /> Selesaikan Kendala</button></div></article> : <form className="form-card compact incident-report-form" onSubmit={(event) => { event.preventDefault(); const formElement = event.currentTarget; const fd = new FormData(formElement); void act("reportCourierIncident", { requestId: requestId("incident"), payload: { type: fd.get("type"), detail: fd.get("detail"), delayEstimate: fd.get("delay") } }, "Kendala dilaporkan ke Farmasi.").then(result => { if (result) formElement.reset(); }); }}><SectionTitle title="Laporkan kendala" text="Catat kendala agar Farmasi dapat berkoordinasi. Estimasi hanya informasi dan tidak mengunci penyelesaian." /><div className="form-grid"><Field label="Jenis kendala"><select name="type" required><option value="">Pilih kendala</option>{optionsOf(operationalOptions, "COURIER_INCIDENT_TYPES").map(value => <option key={value}>{value}</option>)}</select></Field><Field label="Estimasi keterlambatan"><select name="delay" required>{optionsOf(operationalOptions, "DELAY_ESTIMATES").map(value => <option key={value}>{value}</option>)}</select></Field><Field label="Detail (opsional)"><textarea name="detail" placeholder="Isi bila ada informasi tambahan, tanpa data pasien" /></Field></div><button className="danger-button incident-submit" disabled={busy}><TriangleAlert /> Laporkan Kendala</button></form>}{actionModals}</div>;
...
function AdminView({ active, data, areas, areaCount, transactionCount, activeAccountCount, accounts, retention, health, currentUser, operationalSettings, onRefresh, onLogout, navigate, show }: { active: string; data: Row[]; areas: Row[]; areaCount: number; transactionCount: number; activeAccountCount: number; accounts: Row[]; retention: Row | null; health: Row; currentUser: AppUser; operationalSettings: Row; onRefresh: () => void; onLogout: () => void; navigate: (id: string) => void; show: (type: "success" | "error", message: string) => void }) {
  ...
  const [backupForm, setBackupForm] = useState({ note: "Backup pengaman dibuat melalui Dashboard Admin", adminPin: "" });
  ...
  async function approveCleanup(event: FormEvent) {
    ...
    if (!manifestId) return show("error", "Data persiapan pembersihan belum tersedia.");
    if (!/^\d{4,6}$/.test(cleanupForm.adminPin)) return show("error", "PIN Admin harus 4–6 angka.");
    if (cleanupForm.confirmation !== "SETUJUI PEMBERSIHAN") return show("error", "Ketik SETUJUI PEMBERSIHAN persis untuk melanjutkan.");
    const result = await invoke<Row>("adminApproveCleanup", { requestId: requestId("cleanup_approve"), manifestId, adminPin: cleanupForm.adminPin, confirmation: cleanupForm.confirmation }, "Pembersihan disetujui setelah backup pengaman dibuat.");
    if (result) { setCleanupResult(result); setCleanupForm({ adminPin: "", confirmation: "" }); }
  }

  async function runCleanupBatch() {
    if (!cleanupDialog) return; const manifestId = String(get(cleanupResult || cleanupDialog, "manifestId"));
    if (!manifestId) return show("error", "Data persiapan pembersihan tidak ditemukan.");
    const result = await invoke<Row>("adminRunCleanupBatch", { requestId: requestId("cleanup_batch"), manifestId, limit: 200 }, "Proses pembersihan dijalankan dengan perlindungan penghapusan aktif.");
  ...
  async function submitBackup(event: FormEvent) {
    ...
    const ok = await act("adminCreateBackup", { kind: "PRECHANGE", note: backupForm.note, adminPin: backupForm.adminPin }, "Backup pengaman dibuat.");
  ...
  const optionLabels: Record<string, string> = {
    PENDING_REASONS: "Alasan Pengantaran Ditunda",
  ...
  const areaModal = areaDialog ? <Modal wide title="Kelola Desa/Kelurahan" text="Identitas master resmi 623 wilayah dikunci. Admin hanya mengatur cakupan layanan dan biaya." onClose={() => !busy && setAreaDialog(null)}>
  ...
  const correctionModal = correctionRow ? <Modal title={"Koreksi " + deliveryCode(correctionRow)} text="Perubahan status membutuhkan alasan dan PIN Admin; seluruhnya tercatat pada jejak audit." onClose={() => !busy && setCorrectionRow(null)}>
  ...
  const cleanupModal = cleanupDialog ? <Modal wide title={`Pembersihan Data Tahun ${cleanupDialog.year}`} text="Tidak ada penghapusan otomatis. Sistem memeriksa keamanan data, membuat backup pengaman, lalu meminta persetujuan Admin." onClose={() => !busy && setCleanupDialog(null)}>
    <div className="cleanup-gates">
      ...
      <div><small>Arsip lengkap</small><strong>{cleanupGates.archiveComplete === false ? "BELUM" : cleanupGates.archiveComplete === true ? "YA" : "—"}</strong></div>
      <div><small>Pekerjaan aktif</small><strong>{cleanupGates.noActiveWorkflow === false ? "ADA" : cleanupGates.noActiveWorkflow === true ? "TIDAK" : "—"}</strong></div>
      ...
      <div><small>Backup pengaman</small><strong>{cleanupGates.backupHealthy === false ? "GAGAL" : cleanupGates.backupHealthy === true ? "SEHAT" : "SAAT PERSETUJUAN"}</strong></div>
      <div><small>Pemeriksaan akhir</small><strong>{cleanupGates.reconciliationPass === false ? "GAGAL" : cleanupGates.reconciliationPass === true ? "LULUS" : "SAAT PERSETUJUAN"}</strong></div>
    </div>
    {(cleanupStatus === "" || cleanupStatus === "ELIGIBLE" || cleanupStatus === "BLOCKED") && <form className="modal-form" onSubmit={prepareCleanup}>
      <div className="policy-note"><ShieldCheck /><div><strong>Tahap 1 — Periksa kesiapan</strong><p>Sistem memeriksa kelengkapan arsip, pekerjaan aktif, dan integritas data. Belum ada data yang dihapus.</p></div></div>
    ...
    {cleanupStatus === "PREPARED" && <form className="modal-form" onSubmit={approveCleanup}>
      <div className="policy-note warning"><TriangleAlert /><div><strong>Tahap 2 — Persetujuan final</strong><p>Saat disetujui, sistem membuat backup pengaman terlebih dahulu. Pembersihan kemudian berjalan bertahap.</p></div></div>
      <Field label="Konfirmasi" hint="Ketik persis: SETUJUI PEMBERSIHAN"><input required value={cleanupForm.confirmation} onChange={(e) => setCleanupForm({ ...cleanupForm, confirmation: e.target.value })} /></Field>
      <PinInput label="PIN Admin" value={cleanupForm.adminPin} onChange={(adminPin) => setCleanupForm({ ...cleanupForm, adminPin })} />
      <footer className="modal-actions"><button type="button" className="secondary-button" disabled={busy} onClick={() => setCleanupDialog(null)}>Batal</button><button className="danger-button" disabled={busy || cleanupForm.confirmation !== "SETUJUI PEMBERSIHAN"}><ShieldCheck /> Setujui Pembersihan</button></footer>
    </form>}
    {(cleanupStatus === "APPROVED" || cleanupStatus === "RUNNING") && <div className="modal-form">
      <div className="policy-note"><RefreshCw /><div><strong>Tahap 3 — Proses pembersihan</strong><p>Sistem memproses data secara bertahap agar tidak mengganggu operasional.</p></div></div>
  ...
  const healthHeld = Boolean(get(health, "operationalHold")) || healthStatus === "OPERASIONAL DITAHAN";
  const healthReview = !healthHeld && get(health, "adminActionRequired") === true;
  const healthOk = !healthHeld && !healthReview;
  const publicHealthStatus = healthHeld ? "Layanan Dihentikan Sementara" : healthReview ? "Ada Pengaturan yang Perlu Ditinjau" : "Layanan Berjalan Normal";
  const publicHealthText = healthHeld ? "Transaksi baru sedang ditahan sementara. Hubungi Tim IT/SIMRS atau buka Mode Teknisi bila diperlukan." : healthReview ? "Ada pengaturan operasional yang memang memerlukan tindakan Admin." : "MELESAT siap digunakan untuk operasional Farmasi dan Kurir.";
  return <div className="content-stack"><section className={`health-banner ${healthOk ? "ok" : "bad"}`}><div><span>{healthOk ? <ShieldCheck /> : <TriangleAlert />}</span><div><small>KONDISI LAYANAN</small><h2>{publicHealthStatus}</h2><p>{publicHealthText}</p></div></div><Badge status={publicHealthStatus} /></section><div className="stats-grid"><StatCard icon={<Users />} label="Akun aktif" value={activeAccountCount || accounts.filter((a) => get(a, "active", "Aktif") !== false).length} /><StatCard icon={<MapPin />} label="Desa/Kelurahan" value={areaCount || areas.length} tone="green" /><StatCard icon={<PackageOpen />} label="Transaksi operasional" value={transactionCount || data.length} tone="orange" /><StatCard icon={<Archive />} label="Kebijakan arsip" value="DISIMPAN" note="Selama aplikasi digunakan" tone="purple" /></div><SectionTitle title="Pusat kendali Admin" text="Operasi normal dapat dikelola langsung dari aplikasi." /><div className="admin-shortcuts">{navByRole.ADMIN.slice(1).map((n) => <button key={n.id} onClick={() => navigate(n.id)}><span>{n.icon}</span><div><strong>{n.label}</strong><small>Buka pengaturan</small></div><ChevronRight /></button>)}</div></div>;
}
...
function ManagementView({ active, dashboard, range, setRange, reload }: { active: string; dashboard: Row; range: { startDate: string; endDate: string; reportBasis: string }; setRange: (r: any) => void; reload: () => void }) {
  ...
  if (active === "reports") return <div className="content-stack report-page">
    ...
    <section className="content-stack management-report">
      ...
      <section className="report-sheet report-sheet--summary">
        ...
        <ReportHeading number="02" title="Rekap Pembiayaan" text="Ringkasan biaya yang tercatat saat pendaftaran; pembayaran tidak dilakukan kepada Kurir." />
        <div className="stats-grid finance-stats"><StatCard icon={<WalletCards />} label="Total tarif layanan" value={money(finance.originalTariff)} note="Berbayar penuh + nilai sebelum subsidi" /><StatCard icon={<ShieldCheck />} label="Ditanggung subsidi RSUD" value={money(finance.subsidyAmount)} tone="green" /><StatCard icon={<WalletCards />} label="Tagihan pasien" value={money(finance.patientCharge)} tone="orange" /><StatCard icon={<Check />} label="Diterima di Farmasi" value={money(finance.collectedAtPharmacy)} note={Number(finance.legacyUnverifiedAmount || 0) > 0 ? `${money(finance.legacyUnverifiedAmount)} belum terverifikasi` : "Tercatat lunas"} tone="purple" /></div>
        <ReportTable title="Komposisi Layanan Berdasarkan Pembiayaan" headers={["Kategori", "Layanan"]} rows={[
          ["Gratis", Number(finance.freeServices || 0)],
          ["Berbayar penuh", Number(finance.paidServices || 0)],
          ["Mendapat subsidi RSUD", Number(finance.subsidizedServices || 0)],
        ]} />
      </section>
      ...
      <section className="report-sheet report-sheet--break">
        <ReportHeading number="06" title="Evaluasi Pengantaran & Kendala" text="Pengantaran ulang, penundaan, penyebab gagal, dan kendala Kurir sebagai bahan perbaikan operasional." />
  ...
  return <div className="content-stack management-dashboard"><section className="executive-hero"><div className="executive-copy"><span className="executive-kicker"><Building2 /> RINGKASAN EKSEKUTIF</span><h2>Kinerja layanan pengantaran obat</h2><p>Ikhtisar operasional MELESAT untuk pengambilan keputusan Manajemen RSUD Provinsi NTB.</p><div className="executive-period"><CalendarDays /><span><small>PERIODE LAPORAN</small><strong>{periodLabel}</strong></span></div></div><div className="executive-art"><span>MELESAT</span><img src="./assets/maskot-melesat.png" alt="Maskot MELESAT" /></div></section>
    ...
    {dashboard.meta?.archiveOnly && <div className="policy-note"><Archive /><div><strong>Data berasal dari arsip</strong><p>Periode ini dihitung dari ringkasan KPI anonim karena detail operasional lama telah dibersihkan.</p></div></div>}
  </div>;
}

function SimpleTrendChart({ data }: { data: Row[] }) {
  const values = data.map(row => Math.max(0, Number(row.total || 0)));
  const max = Math.max(1, ...values);
  const width = 1000, height = 250, left = 42, top = 16, bottom = 32, usableW = width - left - 16, usableH = height - top - bottom;
  const points = values.map((value, index) => {
    const x = left + (values.length <= 1 ? usableW / 2 : index / (values.length - 1) * usableW);
    const y = top + usableH - value / max * usableH;
    return [x, y] as const;
  });
  const line = points.map(([x,y]) => `${x.toFixed(1)},${y.toFixed(1)}`).join(" ");
  const area = points.length ? `${left},${top + usableH} ${line} ${left + usableW},${top + usableH}` : "";
  const first = String(data[0]?.date || ""); const last = String(data[data.length - 1]?.date || "");
  return <div className="simple-trend-chart" role="img" aria-label={`Tren pengantaran, maksimum ${max} layanan per hari`}>
    <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
      {[0,.25,.5,.75,1].map(r => <line key={r} x1={left} x2={left+usableW} y1={top+usableH*r} y2={top+usableH*r} className="trend-grid" />)}
      <polygon points={area} className="trend-area" /><polyline points={line} className="trend-line" />
      {points.map(([x,y],i) => <circle key={i} cx={x} cy={y} r="4" className="trend-dot"><title>{String(data[i]?.date || "")} • {values[i]}</title></circle>)}
      <text x="4" y={top+6}>{max}</text><text x="4" y={top+usableH+4}>0</text>
      <text x={left} y={height-6}>{first}</text><text x={left+usableW} y={height-6} textAnchor="end">{last}</text>
    </svg>
  </div>;
}
...
function AppShell({ user, onLogout }: { user: AppUser; onLogout: () => void }) {
  ...
  const [pharmacyAreasLoaded, setPharmacyAreasLoaded] = useState(false);
  ...
  const { toast, show } = useToast();
  // Farmasi/Kurir workspace tidak di-refresh hanya karena pindah tab lokal. Admin tetap lazy per menu.
  const activeForRefresh = user.role === "ADMIN" ? active : "";
  const ensurePharmacyAreas = useCallback(async () => {
    if (user.role !== "FARMASI" || pharmacyAreasLoaded) return;
    const result = await callFunction<{ areas: Row[] }>("pharmacyRegistrationData");
    setAreas(result.areas || []);
    setPharmacyAreasLoaded(true);
  }, [pharmacyAreasLoaded, user.role]);
  ...
  const refresh = useCallback(async (quiet = false) => {
    ...
    try {
      if (user.role === "MANAJEMEN") {
        ...
        const workspace = await callFunction<{ rows: Row[]; incidents: Row[]; operationalOptions: OperationalOptions }>("pharmacyWorkspaceData");
        setDeliveries(workspace.rows || []); setIncidents(workspace.incidents || []); setOperationalOptions(workspace.operationalOptions || {});
        ...
        } else if (activeForRefresh === "accounts") {
        ...
        } else if (activeForRefresh === "areas") {
        ...
        } else if (activeForRefresh === "transactions") {
        ...
        } else if (activeForRefresh === "settings") {
        ...
        } else if (activeForRefresh === "archive") {
  ...
  }, [range, show, user.role, activeForRefresh]);

  useEffect(() => { refresh(); }, [refresh]);
  useEffect(() => {
    if (user.role !== "FARMASI" || pharmacyAreasLoaded || busy) return;
    const timer = window.setTimeout(() => { void ensurePharmacyAreas().catch(() => undefined); }, 2500);
    return () => window.clearTimeout(timer);
  }, [user.role, pharmacyAreasLoaded, busy, ensurePharmacyAreas]);
  useEffect(() => {
    if (!["FARMASI", "KURIR"].includes(user.role)) return;
    let dateKey = todayKey();
    const check = () => {
      const next = todayKey();
      if (next !== dateKey) { dateKey = next; void refresh(true); }
    };
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      deltas.forEach(delta => {
        ...
        if (type === "DELIVERY" && id) setDeliveries(current => kind === "REMOVE" ? current.filter(row => rowId(row) !== id) : (() => { let found=false; const next=current.map(row => { if(rowId(row)!==id)return row; found=true; return mergeVersioned(row, record); }); return found?next:[record,...next]; })());
        if (type === "INCIDENT" && id) {
          setIncidents(current => kind === "REMOVE" ? current.filter(row => String(get(row,"id","incidentId")) !== id) : (() => { let found=false; const next=current.map(row => { if(String(get(row,"id","incidentId"))!==id)return row; found=true; return mergeVersioned(row, record); }); return found?next:[record,...next]; })());
